...
def construir(a):
    ...
    X, Y, P, R, C = [], [], [], [], []          # mel, descriptores, persona, real(1)/clon(0), clip
    t0 = time.time()
    for n, f in enumerate(filas):
        clip = Path(f["fichero"]).stem
        fuentes = [(Path(a.dataset) / f["fichero"], 1)]
        clon = Path(a.clones) / f["hablante"] / Path(f["fichero"]).name
        if a.clones and clon.exists():
            fuentes.append((clon, 0))
        for ruta, real in fuentes:
...
def main():
    ...
    claves = sorted({(C[i], R[i]) for i in iv})
    por_clip = {k: [j for j, i in enumerate(iv) if (C[i], R[i]) == k] for k in claves}
    ...
    reales = [k for k in claves if k[1] == 1]
    ...
    auc_clip = auc([k[1] for k in con], [rea[por_clip[k]].mean() for k in con])
    ...
    puerta = {"estilo_r2_positivos": sum(v > 0 for v in r2.values()), "estilo_de": len(r2),
              ...
              "real_clon_auc_clip": auc_clip}
    puerta["pasa"] = bool(puerta["estilo_r2_positivos"] > len(r2) / 2
                          and puerta["persona_acierto_clip"] > puerta["persona_azar"]
                          and (auc_clip > 0.7 if auc_clip == auc_clip else False))
    ...
    print(f"   real/clon: AUC por clip {auc_clip:.3f} ({len(con)} clips)")
