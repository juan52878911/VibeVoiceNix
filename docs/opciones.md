...
| `ruidoArranque` | `nullOr int` | `1` | Semilla del ruido de los primeros fotogramas de cada locución, contra la música de fondo que el modelo inventa en intros. **La 1 es el defecto desde el 16-09-2026**: en OpenVINO deja la música en 0/72 clips (32/72 sin ella), con −1,4 puntos de WER y +0,27 de UTMOS, a cambio de 0,016 de identidad. `null` = apagado, el audio de antes bit a bit. La semilla buena depende del motor y de la voz; la de la ficha `<voz>.json` manda sobre este defecto (ver `ruido_arranque` en [api.md](api.md)). |
...
| `VIBEVOICE_RUIDO_ARRANQUE` | `1` por defecto; vacía o `0` lo apaga. Semilla del ruido de arranque (música inventada). Es `services.voz-stream.ruidoArranque`; por petición, `ruido_arranque` |
