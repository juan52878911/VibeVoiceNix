...
| `ruidoArranque` | `nullOr int` | `null` | Semilla del ruido de los primeros fotogramas de cada locución, contra la música de fondo que el modelo inventa en intros. `null` = apagado, el audio de antes bit a bit. No hay una semilla buena para todos los motores (la 7: 0/72 en torch fp32, 18/72 en OpenVINO; ver `ruido_arranque` en [api.md](api.md)); se usa por voz o por petición, que mandan sobre este defecto. |
...
| `VIBEVOICE_RUIDO_ARRANQUE` | vacía por defecto (apagado); `0` también lo apaga. Semilla del ruido de arranque (música inventada). Es `services.voz-stream.ruidoArranque`; por petición, `ruido_arranque` |
